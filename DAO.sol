...
// som kind of VIP ID Called "Twin Promess"
struct TwinPromess {
    uint256 id;
    address owner;
    uint256 tokenAmount;
...
// a single class type (mybe for offering more infos about each class type)
struct Class {
    ClassType _class;
...
// each member have a deferent member card and contains all the infos about the member 
// (didn't get : uncleList) ??
// pending commusion ??
struct Member {
...
// community branches(
// multiple classes can upgrade from current class once you become a member by buying coins
// system of burning tokens to encrease the value of tokens
// 
// )
...
// a token called "Gven Token" and it's burnable // there is a function that is commented out (balanceOf)
...
// an NFT called "Twin Token Promess"
...
// another token named "Twin Token"
...
//----------------------------------------------------------contracts-----------------------------------------------------------
//------------- "note:" ==> "?"
// the DAO is called "Gven Smart Community"
// to join the cumunity you have to buy some Gven Token using BNB and you'll get PGven NFT as well
// note: who is creators and creatorsWallet?
// note: there is a refund possibility ?
// note: what is gmw ? "it's set to: False" => this turns True once a function setgenesis() is called
// stwrdBook contains all the members cards
// has 8 types of classes, each with deferent attrubute to them
// each class type has a deferent rewards
// there is an invite package ! wich can be bought, but what it do?
// note: what is _frequency ?
// there is a service wich it can be bought - note: what is the serveces
// there is 4 types of main classes
// note: what is _mwUncle ?
// only owner or admin and other members can add a new associate(member)
// a new member has his own wallet (it can be lockable depends on the type of class he's in)
// note: getPrice() is confusing and don't make sense to me
//
//
//
//
...
contract Community is ICommunity {
    ...
    uint256 _assocCounter; //how much members are joined so far
    uint256 _totalReceived; // total balance of the contract ?  
    uint256 _cycleIncreaseCounter; // number of cycels so far
    uint256 _minCycleToBurn; 
    uint256 _currentCycle;
    uint256 _soldAmount; 
    // uint256 _totalSales;
    uint256 _refundReserve; // in case of an event !
    ...
    mapping(ClassType => Class) classes; //ClassType(int) =>Class // it should have 8 types
    ...
    // trying to use block.tampStamp as a random number for generating an id ??
    ...
    constructor(Splitter _creatorsWallet, address adminAddress_) {
        ...
        _serviceBuyGvenAmount = 15_84978000_0000000000; // valor minimo q um servicço pode oferecer é ...
        ...
        initClass(
            ...
            0.0634000_0000000000, /* memberShip value */
...
// just interfaces for creating an NFT and an ERC20 tokon ---------------------------------------------------------------------------
// users can have multiple promesses
// note: the burn machanisme does not aim for increasing value
// zero address (address(0)) is not allowed to do any transactions -used by requier a lot
// note: the burning of a token is by signing the token to the zero address
// these functions are/will be called after the deployment passing on the comunity/twinToken/gvenToken address because : they're never used by this contract
// is promess like the URI in an ordenary NFT contract?
// note: need to know hen to use : storage 
// note: didn't fully understand _checkOnERC721Received()
// an ERC20 token interface
contract ERC20 is Context, IERC20 {
    ...
    function _mint(address account, uint256 amount) internal virtual {
        ...
        require(
            _totalSupply.a(amoddunt) <= _maxSupply,
...
contract ERC721 is Context, ERC165, IERC721 {
    ...
    mapping(address => uint256[]) internal _userPromesses; // how much promesses every user has (users can have multiple promesses)
    ...
    function _mint(
        ...
        ); // only community can mint / and you can join the community by buying some Gven tokens 
        require(to != address(0), "ERC721: mint to the zero address");

        _beforeTokenTransfer(address(0), to, tokenId); // <- what is the usecase of this function ??
     ...
     * Emits a {Transfer} event.
     */
    // the burn machanisme does not aim for increasing value
    ...
    function _transfer(
        ...
        // the burning of a token is by signing the token to the zero address
        _approve(address(0), tokenId);
    ...
    // if you have the id get the user, if you have the user get the id
...
//-----------------------------------------------------------------------------------------------------------------------------------
// GVEN20 wil make GvenToken and TwinToken go hand in hand in terms of transactions
// any Gven/Twin token holder can reanounce ownership of the GVENERC20 token
//
//
//
//
// note: didn't fully understand where virtual and override are used for
// what is a creators wallet 
// made a personlized ERC20 token for GvenToken called "GVENERC20" to work with "GvenToken and TwinToken" -> how ?
// he personlized the ERC721 token contract to work with "TwinTokenPromess" -> how ?
// in this comunity there is 3 tokens : ---erc20:"GvenToken"/ NFT:"TwinTokenPromess"/ erc20, erc20burnable"TwinToken"---
// the "Gven Token" !!*****************************************************************************************************************
contract GVENERC20 is ERC20 {
    ...
    // this function to increament all the information neccesiry
    function _beforeTokenTransfer(
        address from,
        address to,
        uint256 amount
...
// what twinPromess.resolvePromise is ?
// 3 layers tokens (a token inside a token inside a token) for GvenToken and TwinToken
// (super.transfer(recipient, amount);) how this works ? / why it looks for the balance of the twintoken when making a GvenToken transfer ?
// there is a max amount of token that the creator can be given by anyone (mybe as a donation)
//
//
//
//
...
// the "Twin Token" and "Twin Token Promess"*******************************************************************************************
...
// what is when ?
// note: didn't fully understand partialTransferFrom() and resolvePromise()
//
//
//
//
...
// the member's personalized wallet inside the DAO/community
// the wallet can be locked and unlocked by ...
// when called set locked/not, the owner(new member?), 
// community root?!!!!!!??
// (it transfers all the funds from the owners wallet to the owner's community wallet!!! ) inside the contract
// how Icommunity root operate
// q: when you say payable(owner).transfer(address(this).balance) you mean just the balance of this contract or the hole contract(whic have all the other contracts)?
// what it means (root.initAssoc{value: msg.value}(msg.sender);) in community contract
    ...
    IMemberWallet 
{
        ...
        onlyLockerUnlocker
    {
        ...
        if (msg.value > 0) {
            payable(owner).transfer(msg.value); // <- it transfers all the funds from the owners wallet to the owner's community wallet!!! 
        }
    }
...
// need some more clarification and understanding in Lokcer and UnLocker*************
contract Locker {
    IMemberWallet mw;
...
// what is Partner?
// only members can become partners
// only owner or attorny can add a new partner
// Owner or attorney can change parters wallet if necessary
// the contract has a (member/owner/attorney/partners)
// any one can change his wallet
// each partner will get a share of the contract balance depending on the percentage set by eather the attorny or the owner
// there is a silverBollut function that drains all the balance of the contract and transfer it to the owner !!
contract Splitter {
    using SafeMath for uint256;
    ...
    // check if this wallet is one of the partners or not returns(address of the partner)
